...
### 4.0 Wire alarm notifications (prerequisite for production)

**Files:** `infra/lib/doc-platform-stack.ts`

From architecture review: all CloudWatch alarms + SNS topic have NO subscriptions. Alarms fire into a void. Before production launch:
- Create an SNS topic for operational alarms (or reuse existing `github-sync-alarms` topic)
- Add `alarmActions` to all 6 CloudWatch alarms (CPU, memory, 5xx)
- Subscribe at minimum an email address; consider PagerDuty/Opsgenie integration later
- The Lambda DLQ alarm (line 835) already has an SNS action but the topic has no subscribers
