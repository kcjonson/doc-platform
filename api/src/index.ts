...
app.get('/api/epics/:id', async (c) => {
	...
	// Validate UUID
	if (!isValidUUID(id)) {
		return c.json({ error: 'Invalid epic ID format' }, 400);
	}
...
app.post('/api/epics', async (c) => {
	...
	try {
		...
		// as the application handles ordering by rank with fallback to id
...
app.put('/api/epics/:id', async (c) => {
	...
	try {
		...
		if (updates.length === 0) {
			...
			return c.json(dbEpicToApi(epic));
		}
